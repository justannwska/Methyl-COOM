...
## 03_Linear modeling on non-B-cell specific  CpGs.R
## 
## 
##
########################################################################################################################
...
# for full methylation table
#meth.data.full <- readRDS(file.path(data.dir, "450k.meth.table.avg.bcells.NGCLL.samples-2019-11-03.RDS")) 
#for parts of methylation table (due to size restriction - this file is deposited on github)
meth.data.full <- readRDS(file.path(data.dir, "part.450k.meth.table.avg.bcells.NGCLL.samples-2019-11-03.RDS")) 




########################################################
## extract non-B-cell specific CpGs
non.bcell.specific.probes <- meth.data.full$cg[!(meth.data.full$cg %in% as.character(meth.data.all$cg))]
meth.non.bcell.specific.probes <- meth.data.full[!(meth.data.full$cg %in% as.character(meth.data.all$cg)), ]
rownames(meth.non.bcell.specific.probes) <- meth.non.bcell.specific.probes$cg
meth.non.bcell.specific.probes <- meth.non.bcell.specific.probes[, !colnames(meth.non.bcell.specific.probes) == "cg"]
...
meth.non.bcell.specific.probes$intmbcs.mgzs <- rowMeans(meth.non.bcell.specific.probes[, c("intMBCs", "MGZs")])
Meth.Bcells <- meth.non.bcell.specific.probes[,c("NBCs", "GCFs", "loMBCs", "intmbcs.mgzs", "hiMBCs")]
...
#saving a file
saveRDS(linear.model, paste0(data.dir, "/linear.model.on.non.B.cell.specific.CpGs.Bcells", Sys.Date(), ".RDS"))
...
Meth.CLLs.all <- meth.non.bcell.specific.probes[ , Distances.CLL.all$sample]
...
#saving a file
saveRDS(Matrix.for.relative.meth.CLL.group1, paste0(data.dir, "/linear.model.on.non.B.cell.specific.CpGs.CLLs-",Sys.Date(), ".RDS"))
...
saveRDS(rel.meth, paste0(data.dir, "//Relative.meth.linear.model.on.non.B.cell.specific.CpGs.CLLs-",Sys.Date(), ".RDS"))
